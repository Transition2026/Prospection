...
export default function SearchForm({ onSearch, onCancel, loading, categoryFilter, onCategoryChange, exclureGroupes, onExclureGroupesChange, filterLegalHeadquarters, onFilterLegalHeadquartersChange }) {
  ...
  function submit(event) {
    ...
    const params = {
      ...
      limit: limit === 'all' ? 'all' : Math.min(Math.max(Number(limit) || 25, 1), 10000),
      per_page: 25,
      tranche_effectif_salarie: [
        ...(categoryFilter.micro ? ['01', '02', '03'] : []),
        ...(categoryFilter.pme ? ['11', '12', '21', '22', '31'] : []),
        ...(categoryFilter.grande ? ['32', '41', '42', '51', '52', '53'] : []),
      ],
    };
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-5">
        ...
        <label className="text-sm text-gray-600">Objectif <select value={limit} onChange={(event) => setLimit(event.target.value)} className="ml-2 rounded-lg border border-gray-300 px-2 py-1.5"><option value="25">25</option><option value="100">100</option><option value="500">500</option><option value="2000">2 000</option><option value="10000">10 000</option><option value="all">Jusqu'à épuisement (max. 10 000)</option></select></label>
        {loading && <button type="button" onClick={onCancel} className="rounded-xl border border-red-300 px-4 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-50">Arrêter</button>}
